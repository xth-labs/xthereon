    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // XThereon: Only support BIP125 RBF when -mempoolreplacement arg is set
...
{
    LOCK(cs_main);
      LogPrintf("*** LoadGenesisBlock CALLED ***\n");
    ...
    if (m_blockman.m_block_index.count(chainparams.GenesisBlock().GetHash()))
        return true;
	LogPrintf("*** Writing genesis block to disk ***\n");

    try {
        const CBlock& block = chainparams.GenesisBlock();
        FlatFilePos blockPos = SaveBlockToDisk(block, 0, chainparams, nullptr);
      LogPrintf("*** SaveBlockToDisk returned: nFile=%d, nPos=%d ***\n", blockPos.nFile, blockPos.nPos);  
      if (blockPos.IsNull())
