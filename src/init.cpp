 ...
 */
static const char* BITCOIN_PID_FILENAME = "xthereond.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/xthereon-project/xthereon>";
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if xthereon is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if xthereon is started while in a temporary directory.\n",
    ...
    LogPrintf("XTH: starting CPU miner thread\n");
threadGroup.create_thread([&] {
    TraceThread("xth-miner", [&] {
        XTHBitcoinMiner(chainparams, g_chainman);
    });
});
    ...
    // XTH: wire mempool globally for miner threads
    g_mempool = node.mempool.get();
    LogPrintf("XTH: g_mempool assigned OK\n");

    if (node.mempool) {
         int ratio = std::min<int>(std::max<int>(args.GetArg("-checkmempool",
