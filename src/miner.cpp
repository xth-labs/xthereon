...
std::atomic<bool> g_fGenerateBitcoins{false};
static boost::thread_group* g_minerThreads = nullptr;

// Single mining worker thread
void XTHBitcoinMiner(const CChainParams& chainparams, ChainstateManager& chainman)
{
    try {
        while (true) {

            // Mining enabled?
            if (!g_fGenerateBitcoins.load()) {
            ...
            // Wallet available?
            CWallet* pwallet = GetWallets().empty() ? nullptr : GetWallets()[0].get();
            if (!pwallet) {
                std::this_thread::sleep_for(std::chrono::milliseconds(1000));
                continue;
            }
            ...
            CTxDestination dest;
            if (!reserveDest.GetReservedDestination(dest, false)) {
                std::this_thread::sleep_for(std::chrono::milliseconds(1000));
                continue;
            }
            ...
            // Coinbase must exist
            if (pblock->vtx.empty()) {
                LogPrintf("XTH miner BUG: block has no coinbase\n");
                continue;
            }
            ...
            while (g_fGenerateBitcoins.load()) {
                ...
                // 🔑 FIX: recompute merkle root BEFORE hashing
                pblock->hashMerkleRoot = BlockMerkleRoot(*pblock);
                ...
                if (CheckProofOfWork(hash, pblock->nBits, chainparams.GetConsensus())) {
                    ...
                    bool accepted = chainman.ProcessNewBlock(
                        chainparams, shared_block, true, &newBlock
                    );

                    if (accepted) {
                        reserveDest.KeepDestination();
                        LogPrintf("XTH miner: BLOCK ACCEPTED %s\n", hash.ToString());
                    } else {
                        LogPrintf("XTH miner: block rejected %s\n", hash.ToString());
                    }

                    break; // rebuild template
                }
                ...
                // Debug progress
                if (++iter % 5'000'000 == 0) {
                    LogPrintf("XTH miner: hashing… nonce=%u\n", pblock->nNonce);
                }
        ...
        LogPrintf("XTH miner exception: %s\n", e.what());
    }
