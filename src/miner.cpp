...
// ============================================================================
// XTH: Simple CPU miner implementation
// ============================================================================

// Global mempool is defined in validation.cpp
extern CTxMemPool* g_mempool;
// Mining control state
static std::atomic<bool> g_fGenerateBitcoins{false};
static boost::thread_group* g_minerThreads = nullptr;

// Single mining worker thread
static void XTHBitcoinMiner(const CChainParams& chainparams, ChainstateManager& chainman)
{
    LogPrintf("XTH miner: CPU mining thread started\n");

    static unsigned int nExtraNonce = 0;   // <-- REQUIRED for merkle correctness

    try {
        while (g_fGenerateBitcoins.load()) {

            if (g_mempool == nullptr) {
                LogPrintf("XTH miner: g_mempool is null inside miner thread, sleeping...\n");
                std::this_thread::sleep_for(std::chrono::milliseconds(1000));
                continue;
            }

            CScript scriptPubKey; // empty script for now
            BlockAssembler assembler(*g_mempool, chainparams);
            std::unique_ptr<CBlockTemplate> pblocktemplate =
                assembler.CreateNewBlock(scriptPubKey);
            ...
            while (g_fGenerateBitcoins.load()) {
                ...
                if (CheckProofOfWork(hash, pblock->nBits, chainparams.GetConsensus())) {
                    ...
                    bool newBlock = false;
                    bool accepted =
                        chainman.ProcessNewBlock(chainparams, shared_block, true, &newBlock);

                    if (accepted) {
                        LogPrintf("XTH miner: new block found %s\n", hash.ToString());
                    } else {
                        LogPrintf("XTH miner: block rejected %s\n", hash.ToString());
                    }

                    break; // rebuild new template after found block
                }
                ...
                if ((pblock->nNonce & 0xFFF) == 0) {
                    ...
                    if (!tip || tip->GetBlockHash() != pblock->hashPrevBlock) {
                        break;
                    }
                }
            } // end nonce loop
        ...
        LogPrintf("XTH miner: exception in miner thread: %s\n", e.what());
    }

    LogPrintf("XTH miner: CPU mining thread exiting\n");
}

// ============================================================================
// XTH: Start/stop CPU mining controller
// ============================================================================
                      ...
                      ChainstateManager& chainman)
{
    ...
    g_fGenerateBitcoins.store(false);

    if (g_minerThreads != nullptr) {
        g_minerThreads->interrupt_all();
        g_minerThreads->join_all();
        delete g_minerThreads;
        g_minerThreads = nullptr;
    }

    if (!fGenerate || nThreads == 0) {
        LogPrintf("XTH miner: CPU mining disabled\n");
        return;
    }
    ...
    g_fGenerateBitcoins.store(true);

    g_minerThreads = new boost::thread_group();

    for (int i = 0; i < nThreads; ++i) {
        g_minerThreads->create_thread([&chainparams, &chainman]() {
            XTHBitcoinMiner(chainparams, chainman);
        });
    }

    LogPrintf("XTH miner: CPU mining enabled with %d thread(s)\n", nThreads);
}
