...
The XThereon Core daemon can be configured to act as a trusted "border
router", implementing the xthereon wire protocol and relay, making
...
The ZeroMQ feature in XThereon Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building xthereond:
...
For instance:
    ...
    $ xthereond -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/xthereond.tx.raw \
...
These options can also be provided in xthereon.conf.
...
From the perspective of xthereond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into xthereond directly. Furthermore, no information is
...
using. XThereond appends an up-counting sequence number to each
